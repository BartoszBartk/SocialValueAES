...
patches-own [
  ;;parcel attributes:
  reserv ;;Boolean, protection status (if YES, no agricultural production possible & OCCUP = TRUE)
  conserv ;;Boolean, conservation status of parcel
  contrib-margin ;;contribution margin / opportunity costs of conservation
  occup ;;Boolean, occupancy by protected species
  occup-next ;;Boolean, occupancy at next time step
  occup-neighb ;;number of occupied neighbours
  prob-occup ;;probability of being occupied given colonization and extinction
  ;;farmer attributes:
...
to setup
  ca
  ;;determine reserved patches
  ask n-of n-reserved patches [
    set reserv true
    set occup true
    set pcolor green
  ]
  ask patches with [reserv != true] [
    set reserv false
...
to check-behav
  ...
  if behav = "maximizer" [
    ask patches [
      set wta contrib-margin
    ]
  ]
  ;;beyond-max variant: for half of all farmers, willingness to accept exceeds opportunity costs by 10%
  if behav = "beyond-max" [
    ask patches [
      set wta contrib-margin
    ]
    ask n-of (0.5 * count patches) patches [
      set wta (1.1 * contrib-margin)
    ]
  ]
end
...
to colonize
  ;;colonize neighbouring patches + extinction events
  ask patches with [conserv = true] [
    let col (1 - (1 - prob-col) ^ occup-neighb)
    ifelse random-float 1 <= col [
      set occup-next true
    ][
      set occup-next occup
    ]
    if random-float 1 <= prob-ext [
      set occup-next false
    ]
  ]
  ask patches with [reserv = false] [
    set occup occup-next
    ifelse conserv = true [
      set pcolor orange
    ][
      set pcolor red
...
to calc-welfare
  ...
  set social-welfare social-value * count patches with [occup = true] + sum [contrib-margin] of patches with [conserv = false] - budget
